...
def insertVariblesIntoTable(company_title, company_logo, url, description, product, location, founded, website,
    ...
    try:
        ...
        record = (
        company_title, url, company_logo, description, product, location, location, founded, website, linkedin,
        facebook, twitter)
...
for x in range(1, 2):
    response = requests.get(base_url + '?model=Companies&page=' + str(x) + '&size=9&term=%2A&type=Filters',
                            headers=headers)
    doc = html.fromstring(response.content)

    company_archives = doc.xpath('//div[@class="col-md-4 portfolio-item"]')
    for new in company_archives:
        title = ''.join(new.xpath('.//div[@id="cp_data_box"]/h4/a/text()'))
        assert title
        print(title)
        input('debug')
        ...
        for element in company_page:
            ...
            for i in range(2, 5):
                link_str = "".join(element.xpath('//ul[@class="technologies list-inline"]/li[' + str(i) + ']/a/@href'))
